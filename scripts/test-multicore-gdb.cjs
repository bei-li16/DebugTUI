...
(async()=>{
 try{
  ...
  const group=session('group',base+'[multicore]\nscope="all"\nhalt_peers=true\n'+cores);active.push(group);
  await group.cmd('connect');await group.cmd('continue');
  assert((await group.cmd('status')).result.cores.every(c=>c.state==='RUNNING'));
  await group.cmd('pause');assert((await group.cmd('status')).result.cores.every(c=>c.state==='STOPPED'));
  await group.cmd('select_core',{index:1});await group.cmd('break',{location:'tick'});
  await group.cmd('select_core',{index:0});await group.cmd('continue');
  const groupWait=(await group.cmd('wait_stopped',{timeout_ms:8000})).result;
  assert.equal(groupWait.core,1);assert(groupWait.cores.every(c=>c.state==='STOPPED'));
  let grouped;
  for(let attempt=0;attempt<60;attempt++){
    grouped=(await group.cmd('status')).result;
    if(grouped.core.index===1&&grouped.cores.every(c=>c.state==='STOPPED'))break;
    await new Promise(r=>setTimeout(r,50));
  }
  assert.equal(grouped.core.index,1);assert.equal(grouped.stop_reason,'breakpoint-hit');
  assert(grouped.cores.every(c=>c.state==='STOPPED'),'breakpoint must halt its running peer');
  await group.cmd('delete_break',{number:''});
  const peerPC=(await group.cmd('evaluate',{expression:'$pc'})).result.value;
  await group.cmd('select_core',{index:0});await group.cmd('stepi');await group.cmd('wait_stopped');
  await group.cmd('select_core',{index:1});assert.equal((await group.cmd('evaluate',{expression:'$pc'})).result.value,peerPC);
  await group.cmd('run');await group.cmd('run');assert((await group.cmd('status')).result.cores.every(c=>c.state==='RUNNING'));
  await group.cmd('select_core',{index:0});await group.cmd('pause',{scope:'core'});
  assert.deepEqual((await group.cmd('status')).result.cores.map(c=>c.state),['STOPPED','RUNNING']);
  await group.cmd('pause');assert((await group.cmd('status')).result.cores.every(c=>c.state==='STOPPED'));
  await group.cmd('control_scope',{scope:'core'});await group.cmd('continue');
  assert.deepEqual((await group.cmd('status')).result.cores.map(c=>c.state),['RUNNING','STOPPED']);
  await group.cmd('control_scope',{scope:'all'});await group.cmd('console',{command:'continue'});
  assert((await group.cmd('status')).result.cores.every(c=>c.state==='RUNNING'));
  await group.cmd('pause');await group.close();active.splice(active.indexOf(group),1);
  report.push('group F5 starts both, breakpoint halts peers and focuses cause, step keeps peer PC, repeated Run resumes, explicit core override and scope switching');
  const selected=session('selected-registers',base.replace('[target]','registers=["rax","rip"]\n[target]')+'[multicore]\nscope="all"\n'+cores);active.push(selected);
  await selected.cmd('connect');await selected.cmd('continue');await selected.cmd('pause');
  for(const index of [0,1]){await selected.cmd('select_core',{index});assert.deepEqual((await selected.cmd('status')).result.registers.map(r=>r.name),['rax','rip']);}
  await selected.close();active.splice(active.indexOf(selected),1);report.push('configured automatic register set is respected by both real GDB sessions');
